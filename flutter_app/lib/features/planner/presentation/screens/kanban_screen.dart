...
class _KanbanScreenState extends ConsumerState<KanbanScreen> {
  ...
  void _showEditProjectSheet(BuildContext context, KanbanProject project) {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (ctx) => _EditProjectSheet(project: project),
    );
  }
...
// ── Sheet création projet (multi-step) ───────────────────────
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── Étape 0 : Nom ──
            if (_step == 0) ...[
              Text('Quel est le nom de ton projet ?',
                  style: AppTextStyles.headingSmall(color: textColor)),
              const SizedBox(height: 6),
              Text('Choisis un nom clair et mémorable.',
                  style: AppTextStyles.bodyMedium(
                      color: AppColors.grey400)),
              const SizedBox(height: 20),
              TextField(
                ...
                decoration: _inputDecoration(isDark, 'Ex : Lancer mon podcast'),
                onSubmitted: (_) {
            ...
            // ── Étape 1 : Pourquoi ──
            if (_step == 1) ...[
              Text('Pourquoi ce projet te tient à cœur ?',
                  style: AppTextStyles.headingSmall(color: textColor)),
              const SizedBox(height: 6),
              Text(
                'C\'est ton ancrage. Quand tu doutes, tu te rappelles pourquoi tu agis.',
                style:
                    AppTextStyles.bodyMedium(color: AppColors.grey400),
              ),
              const SizedBox(height: 20),
              TextField(
                ...
                decoration: _inputDecoration(
                    isDark, 'Ex : Pour prouver que j\'ai quelque chose à dire'),
              ),
              const SizedBox(height: 24),
              Row(
                children: [
            ...
            // ── Étape 2 : Date cible ──
            if (_step == 2) ...[
              Text('Une date cible pour ce projet ?',
                  style: AppTextStyles.headingSmall(color: textColor)),
              const SizedBox(height: 6),
              Text(
                'Facultatif, mais utile pour rester aligné avec tes objectifs.',
                style: AppTextStyles.bodyMedium(color: AppColors.grey400),
              ),
              const SizedBox(height: 20),
              GestureDetector(
...
class _EditProjectSheetState extends ConsumerState<_EditProjectSheet> {
  ...
  late final _whyCtrl  =
      TextEditingController(text: widget.project.why ?? '');
  late DateTime? _targetDate = widget.project.targetDate;
  bool _loading = false;
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Modifier le projet',
                style: AppTextStyles.headingMedium(color: textColor)),
            const SizedBox(height: 20),
            _FieldLabel(label: 'Nom du projet', isDark: isDark),
            const SizedBox(height: 8),
            TextField(
              controller: _nameCtrl,
              textCapitalization: TextCapitalization.sentences,
              style: AppTextStyles.bodyMedium(color: textColor),
              decoration: _inputDeco(isDark, 'Nom du projet'),
            ),
            const SizedBox(height: 16),
            _FieldLabel(label: 'Pourquoi ce projet ?', isDark: isDark),
            const SizedBox(height: 8),
            TextField(
              controller: _whyCtrl,
              textCapitalization: TextCapitalization.sentences,
              ...
              decoration: _inputDeco(
                  isDark, 'Ton ancrage motivationnel...'),
            ),
            const SizedBox(height: 16),
            _FieldLabel(label: 'Date cible', isDark: isDark),
            const SizedBox(height: 8),
            GestureDetector(
              onTap: () async {
                final picked = await showDatePicker(
  ...
  Widget build(BuildContext context) {
    ...
    return OutlinedButton(
      ...
      style: OutlinedButton.styleFrom(
        foregroundColor:
            isDark ? AppColors.textDarkMuted : AppColors.grey600,
        side: BorderSide(
            color: AppColors.grey400.withValues(alpha: 0.4)),
        padding: const EdgeInsets.symmetric(vertical: 16),
        shape: const StadiumBorder(),
